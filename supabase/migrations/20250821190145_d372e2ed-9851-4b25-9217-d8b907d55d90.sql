...
ADD COLUMN IF NOT EXISTS party_code TEXT REFERENCES public.parties(code),
...
-- Create RLS policies
CREATE POLICY "Public party data is viewable by everyone" 
ON public.parties FOR SELECT USING (true);

CREATE POLICY "Public manifesto data is viewable by everyone" 
ON public.manifestos FOR SELECT USING (true);
